...
test("right-click on an arrow cycles its heads end → both → start → end", async ({
	page,
}) => {
...
test("right-click box then right-click a second box connects them", async ({
	page,
}) => {
...
test("interior right-clicks leave both endpoint pins auto", async ({
	page,
}) => {
	await seedDoc(page, TWO_BOXES);
	const c = await canvasRect(page);
...
test("edge right-clicks pin the clicked side and exact offset", async ({
	page,
}) => {
	await seedDoc(page, TWO_BOXES);
	const c = await canvasRect(page);
...
test("right-click box then empty space drops a connected box with the editor open", async ({
	page,
}) => {
...
test("right-click on empty space connects from the sole selected box", async ({
	page,
}) => {
	await seedDoc(page, [TWO_BOXES[0]]);
	const c = await canvasRect(page);
...
test("right-click on empty space with nothing armed or selected is a no-op", async ({
	page,
}) => {
	await seedDoc(page, [TWO_BOXES[0]]);
	const c = await canvasRect(page);
...
test("pending-connect hint appears after the first right-click and resets after cancel", async ({
	page,
}) => {
	await seedDoc(page, BOXES_AND_ARROW);
	const c = await canvasRect(page);
	...
	await expect(hint).toContainText(
		'right-click another box to connect it to "this box"',
	);
