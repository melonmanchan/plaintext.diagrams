...
test("pasting an ASCII diagram anchors it at the cursor cell", async ({
	page,
}) => {
	await seedDoc(page, []);
...
test("pasting a Unicode diagram parses boxes, labels, and pinned arrows", async ({
	page,
}) => {
	await seedDoc(page, []);
	const diagram = [
...
test("pasting shapes JSON keeps dashed style and side pins", async ({
	page,
}) => {
	await seedDoc(page, []);
	const c = await canvasRect(page);
...
test("pasting JSON-looking garbage reports the error and leaves the doc unchanged", async ({
	page,
}) => {
	await seedDoc(page, [
		{ type: "box", id: 1, x: 2, y: 2, w: 8, h: 3, text: "KEEP" },
	]);
...
test("Copy JSON output re-imports as an identical diagram", async ({
	page,
}) => {
	await installCopyStub(page);
	await seedDoc(page, [
