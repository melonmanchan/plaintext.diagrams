...
test("dragging a box near another aligns it to the snap guide", async ({
	page,
}) => {
	await seedDoc(page, [
		{ type: "box", id: 1, x: 2, y: 2, w: 12, h: 5, text: "" },
...
test("resizing a lane group keeps contents in their lane slots", async ({
	page,
}) => {
	await seedDoc(page, [
		{
...
test("resize clamps to the label minimum size", async ({ page }) => {
	await seedDoc(page, [
		{ type: "box", id: 1, x: 2, y: 2, w: 18, h: 5, text: "Wide Label" },
	]);
	const c = await canvasRect(page);
...
test("endpoint drag re-attaches with an edge pin; interior drop clears it", async ({
	page,
}) => {
	await seedDoc(page, [
		{ type: "box", id: 1, x: 2, y: 2, w: 12, h: 5, text: "" },
...
test("arrow keys nudge the selection by one cell", async ({ page }) => {
	await seedDoc(page, [
		{ type: "box", id: 1, x: 5, y: 5, w: 6, h: 3, text: "" },
	]);
	const c = await canvasRect(page);
...
test("Delete removes the selection and detaches its arrows", async ({
	page,
}) => {
	await seedDoc(page, [
		{ type: "box", id: 1, x: 2, y: 2, w: 12, h: 5, text: "" },
