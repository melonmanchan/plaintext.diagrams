...
for (const { key, tool } of TOOL_KEYS) {
	test(`'${key}' activates the ${tool} tool`, async ({ page }) => {
		...
		await expect(page.locator("#tools button.active")).toHaveAttribute(
			"data-tool",
			tool,
		);
	});
...
test("Control+d gives a selected group swimlanes with headers", async ({
	page,
}) => {
	await seedDoc(page, [
		{ type: "group", id: 1, x: 2, y: 2, w: 24, h: 8, text: "" },
	]);
	const c = await canvasRect(page);
...
test("[ and ] walk endpoint pins auto → left → right → top → bottom → auto", async ({
	page,
}) => {
	await seedDoc(page, [
		{ type: "box", id: 1, x: 2, y: 2, w: 12, h: 5, text: "" },
	...
	const arrow = async () =>
		(await shapes(page)).find((s) => s.id === 3) as ArrowShape;
...
test("Control+z undoes a drawn box; Shift+z and y redo it", async ({
	page,
}) => {
	await seedDoc(page, []);
	const c = await canvasRect(page);
...
test("'e' copies the diagram and flashes the export button", async ({
	page,
}) => {
	await seedDoc(page, [
		{ type: "box", id: 1, x: 2, y: 2, w: 12, h: 5, text: "" },
