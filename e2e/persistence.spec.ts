...
test("zoom survives a reload", async ({ page }) => {
	...
	expect(await page.evaluate(() => localStorage.getItem("ptd:zoom"))).toBe(
		"1.25",
	);
	await reloadApp(page);
	await expect(reset).toHaveText("125%");
...
test("a new project starts empty and leaves the old doc untouched", async ({
	page,
}) => {
	...
	expect((await projects(page)).map((p) => p.name)).toEqual([
		"Untitled",
		"Second",
	]);
...
test("renaming a project updates the title and the select", async ({
	page,
}) => {
...
test("deleting a project drops its storage keys and falls back", async ({
	page,
}) => {
	const firstId = await page.locator("#project").inputValue();
...
test("legacy vibedraw:* storage migrates to ptd:* on boot", async ({
	page,
}) => {
	await page.evaluate(() => {
		localStorage.clear();
		localStorage.setItem(
			"vibedraw:index",
			JSON.stringify({
				projects: [{ id: "leg1", name: "Legacy Proj" }],
				current: "leg1",
			}),
		);
		localStorage.setItem(
			...
			JSON.stringify({
				seq: 2,
				shapes: [
					{ type: "box", id: 1, x: 2, y: 2, w: 10, h: 4, text: "Legacy" },
				],
			}),
		);
	});
...
test("a corrupted ptd:index boots the demo without throwing", async ({
	page,
}) => {
	const errors: Error[] = [];
