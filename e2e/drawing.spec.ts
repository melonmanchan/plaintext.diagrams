...
test('typing a long label grows the box live and commits widened', async ({ page }) => {
  await seedDoc(page, [{ type: 'box', id: 1, x: 2, y: 2, w: 3, h: 3, text: '' }]);
  const c = await canvasRect(page);
...
test('Escape cancels a label edit and reverts live growth', async ({ page }) => {
  await seedDoc(page, [{ type: 'box', id: 1, x: 2, y: 2, w: 9, h: 3, text: 'Old' }]);
  const c = await canvasRect(page);
...
test('text tool places free text and dblclick promotes it to a box', async ({ page }) => {
  await seedDoc(page, []);
  const c = await canvasRect(page);
...
test('group tool drag creates a frame and typing titles it with a tab', async ({ page }) => {
  await seedDoc(page, []);
  const c = await canvasRect(page);
...
test('double-click on an existing box edits its label', async ({ page }) => {
  await seedDoc(page, [{ type: 'box', id: 1, x: 2, y: 2, w: 12, h: 5, text: 'Old' }]);
  const c = await canvasRect(page);
...
test('typing labels a selected arrow with an embedded caption', async ({ page }) => {
  await seedDoc(page, [
    { type: 'arrow', id: 1, x1: 2, y1: 2, x2: 22, y2: 2, box1: null, box2: null },
  ]);
  const c = await canvasRect(page);
...
test('create-box drag under 3x3 clamps to the minimum size', async ({ page }) => {
  await seedDoc(page, []);
  const c = await canvasRect(page);
  await page.keyboard.press('b');
