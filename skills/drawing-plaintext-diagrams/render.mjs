...
function drawArrow(s, shapes, put, ghost, busy) {
	...
	if (heads !== "start") {
		const a = pts[pts.length - 2],
			b = pts[pts.length - 1];
		const head =
			into2 ?? (b.x > a.x ? ">" : b.x < a.x ? "<" : b.y > a.y ? "v" : "^");
	...
	if (heads !== "end") {
		...
		const tail =
			into1 ??
			(b0.x > a0.x ? ">" : b0.x < a0.x ? "<" : b0.y > a0.y ? "v" : "^");
...
function resolveArrow(a, shapes) {
	...
	const slotOn = (b, side, o, which) => {
		...
		for (const s of shapes) {
			...
			const sb1 = boxOf(s.box1),
				sb2 = boxOf(s.box2);
			if (sb1 === b) {
				const t = sb2 ? center(sb2) : { x: s.x2, y: s.y2 };
				if (
					((pinnable(b, s.side1) ? s.side1 : null) ?? sideFor(b, t)) === side &&
			...
			if (sb2 === b) {
				...
				if (
					((pinnable(b, s.side2) ? s.side2 : null) ?? sideFor(b, t)) === side &&
		...
		const slot = Math.max(
			0,
			entries.findIndex((e) => e.id === a.id && e.which === which),
		);
		return { slot, off: (slot % 2 ? -1 : 1) * Math.ceil(slot / 2) * 2 };
	};
	let p1 = { x: a.x1, y: a.y1 },
		p2 = { x: a.x2, y: a.y2 };
	let ax1 = null,
		ax2 = null;
	let side1 = null,
	...
	const pinnable = (b, side) =>
		side != null &&
		(side === "left" ? b.x >= 1 : side === "top" ? b.y >= 1 : true);
	...
	const obstacles = shapes.filter(
		(s) => s.type === "box" && s !== b1 && s !== b2,
	);
	const hitsBox = (u, v, b) => {
		if (!b) return false;
		return (
			Math.max(u.x, v.x) >= b.x &&
			Math.min(u.x, v.x) <= b.x + b.w - 1 &&
			Math.max(u.y, v.y) >= b.y &&
			Math.min(u.y, v.y) <= b.y + b.h - 1
		);
	};
	const segsClean = (pts2) => {
		for (let i = 0; i < pts2.length - 1; i++) {
			...
			if (i < pts2.length - 2 && hitsBox(u, v, b2)) return false;
	...
	const pathLen = (pts2) => {
		...
		for (let i = 0; i < pts2.length - 1; i++)
			t +=
				Math.abs(pts2[i + 1].x - pts2[i].x) +
				Math.abs(pts2[i + 1].y - pts2[i].y);
		return t;
	};
	const routeAvoiding = () => {
		const outPt = (p, side, k) =>
			side === "right"
				? { x: p.x + k, y: p.y }
				: side === "left"
					? { x: Math.max(0, p.x - k), y: p.y }
					: side === "bottom"
						? { x: p.x, y: p.y + k }
						: side === "top"
							? { x: p.x, y: Math.max(0, p.y - k) }
							: p;
		...
		const spanX1 = Math.min(p1.x, p2.x) - 4,
			spanX2 = Math.max(p1.x, p2.x) + 4;
		const spanY1 = Math.min(p1.y, p2.y) - 4,
			spanY2 = Math.max(p1.y, p2.y) + 4;
		const bs = [
			b1,
			b2,
			...obstacles.filter(
	...
	const finalize = (route) => {
		...
		for (let i = 1; i < route.length; i++) {
			...
			if (route[i].x !== last.x || route[i].y !== last.y) out2.push(route[i]);
		}
	...
	for (const s of shapes) {
		...
		if (s.type === "arrow") {
			...
			for (const ref of [a.box1, a.box2]) {
				if (
					ref != null &&
					!shapes.some((sh) => sh.id === ref && sh.type === "box")
				)
					errors.push(
						`arrow ${a.id} references box id ${ref}, which does not exist`,
					);
			}
			for (const [k, v] of [
				["side1", a.side1],
			...
			if (a.box1 == null && a.box2 == null && a.x1 === a.x2 && a.y1 === a.y2)
				errors.push(
					`arrow ${a.id} needs box1/box2 ids or distinct x1,y1 → x2,y2 coordinates`,
				);
		} else {
			const p = s;
				...
				if (s.type === "box") {
					...
					if (
						...
						!(
							...
							g.lanes.every((l) => typeof l === "string")
						)
					) {
