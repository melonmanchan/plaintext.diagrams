...
const SHAPE_TYPES: Record<string, true> = {
	box: true,
	arrow: true,
	text: true,
	group: true,
	...
	for (const s of shapes) {
		...
		if (s.type === "arrow") {
			...
			if (s.type === "box" || s.type === "group") {
				...
				typeof (s as TextShape).text !== "string" ||
				!(s as TextShape).text
			) {
...
export async function decodeShareLink(
	fragment: string,
): Promise<{ name: string; shapes: Shape[] } | { error: string }> {
	if (!fragment.startsWith(SHARE_VERSION))
		return { error: "unsupported share-link version" };
	let json: string;
	try {
	...
	if (!payload || typeof payload !== "object" || !("s" in payload))
		return { error: "share link is corrupt" };
	const name =
		"n" in payload && typeof payload.n === "string" ? payload.n : "Shared";
	const parsed = parseShapesJson(JSON.stringify({ shapes: payload.s }));
	if (!parsed || parsed.errors.length)
		return { error: parsed?.errors[0] ?? "share link is corrupt" };
