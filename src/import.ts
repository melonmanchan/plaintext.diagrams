...
export function parseAscii(text: string): Shape[] {
	...
	const takeSide = (x: number, y: number) => {
		const c = at(x, y);
		if (c !== "-" && c !== " ") take(x, y);
	};
	const takeEdge = (x: number, y: number) => {
		const c = at(x, y);
		if (c !== "|" && c !== " ") take(x, y);
	};
	...
	function traceArrow(
		...
		const isLine = (c: string, v: [number, number]) =>
			v[1] === 0 ? c === "-" : c === "|" || c === ":";
		...
		for (;;) {
			...
			for (let i = 0; i < maxGap; i++) {
				...
				const continues =
					contin(nx, ny) ||
					(at(nx, ny) === " " && contin(nx + dir[0], ny + dir[1]));
				...
				if (
					(gc === "-" ||
						...
						gc in HEAD_DIR) &&
					!continues &&
					!(letterish(at(gx - dir[0], gy - dir[1])) || letterish(at(nx, ny)))
				) {
					blocked = true;
					break;
				}
			...
			if (dir[1] === 0) {
				...
				for (const [gx0, gy] of gap) {
					...
					while (
						free(lo - 1, gy) &&
						!stopChar(lo - 1) &&
						!(at(lo - 1, gy) === " " && at(lo - 2, gy) === " ")
					)
						lo--;
					while (
						free(hi + 1, gy) &&
						!stopChar(hi + 1) &&
						!(at(hi + 1, gy) === " " && at(hi + 2, gy) === " ")
					)
						hi++;
	...
	for (let pass = 0; pass < 2; pass++) {
		...
		for (const idxs of groupsBySig.values()) {
			...
			const key = (p: { x1: number; y1: number; x2: number; y2: number }) =>
				`${p.x1},${p.y1},${p.x2},${p.y2}`;
			const drawnSet = idxs
				.map((i) => key(drawn[i]))
				.sort()
		...
		arrows.forEach((a, i) => {
			...
			const pinEnd = (which: 1 | 2): void => {
				...
				if (
					isExempt ||
					boxId == null ||
					(which === 1 ? a.side1 : a.side2) != null
				)
					return;
				const b = boxes.find((bb) => bb.id === boxId);
				if (!b) return;
				const dx = which === 1 ? d.x1 : d.x2,
					dy = which === 1 ? d.y1 : d.y2;
				if (
					which === 1
						? sa.x1 === dx && sa.y1 === dy
						: sa.x2 === dx && sa.y2 === dy
