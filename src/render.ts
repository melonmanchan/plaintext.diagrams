...
export function render(): void {
		...
		paintShapes = paintShapes.map((s) => {
			...
			if (s.type === "group") {
				if (app.editingLane != null && s.lanes)
					return {
						...s,
						lanes: s.lanes.map((t, i) => (i === app.editingLane ? "" : t)),
					};
				// space-pad the title so the tab frame keeps its geometry
				return {
					...s,
					text: s.text ? " ".repeat(s.text.split("\n")[0].length) : s.text,
				};
			}
			return { ...s, text: "" };
		});
	...
	for (const s of app.doc.shapes)
		if (s.type === "group")
			ctx.fillRect(s.x * CW, s.y * CH, s.w * CW, s.h * CH);
	...
	if (d && d.mode === "marquee") {
		...
		const w = Math.abs(d.cx - d.sx) + 1,
			h = Math.abs(d.cy - d.sy) + 1;
		...
		for (const g of app.guides) {
			ctx.beginPath();
			if (g.axis === "v") {
				ctx.moveTo(g.px + 0.5, 0);
				ctx.lineTo(g.px + 0.5, H);
			} else {
				ctx.moveTo(0, g.px + 0.5);
				ctx.lineTo(W, g.px + 0.5);
			}
			ctx.stroke();
		}
		ctx.setLineDash([]);
...
export function updateToolbar(): void {
	...
	document.querySelector<HTMLButtonElement>("#redo")!.disabled =
		!app.redoStack.length;
	document.querySelector<HTMLButtonElement>("#delete")!.disabled =
		!app.selection.size;
}
