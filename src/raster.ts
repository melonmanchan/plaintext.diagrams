...
const INTO_HEAD: Record<Side, string> = {
	left: ">",
	right: "<",
	top: "v",
	bottom: "^",
...
const UNI_HEAD: Record<string, string> = {
	">": "▶",
	"<": "◀",
	v: "▼",
	"^": "▲",
...
function anchorFor(
	...
	slot: number,
	off: number,
): { x: number; y: number; axis: "h" | "v"; side: Side } {
...
export function resolveArrow(a: ArrowShape, shapes: Shape[]): ResolvedArrow {
	...
	const slotOn = (
		...
		o: Point,
		which: 1 | 2,
	): { slot: number; off: number } => {
		const vertical = side === "left" || side === "right";
		const cellOf = (t: Point): number =>
			vertical
	...
	let p1: Point = { x: a.x1, y: a.y1 },
		p2: Point = { x: a.x2, y: a.y2 };
	let ax1: "h" | "v" | null = null,
		ax2: "h" | "v" | null = null;
	let side1: Side | null = null,
