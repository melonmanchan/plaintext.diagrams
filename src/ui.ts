...
const HINTS: Record<Tool, string> = {
	select:
		"click: select · shift-click / drag: multi-select · cmd-click: new box · middle-drag: pan · type: edit label · del: delete",
...
function runLayout(
	fn: (shapes: Shape[], only?: ReadonlySet<number>) => void,
): void {
...
function deleteProject(): void {
	...
	if (!app.projects.length)
		app.projects.push({ id: genId(), name: "Untitled" });
...
function onPaste(e: ClipboardEvent): void {
	...
	const dx = clamp(
		anchor.x - minX,
		-minX,
		Math.max(-minX, MAX_COLS - 1 - maxX),
	);
	const dy = clamp(
		anchor.y - minY,
		-minY,
		Math.max(-minY, MAX_ROWS - 1 - maxY),
	);
	for (const s of parsed) {
		if (s.type === "arrow") {
			s.x1 += dx;
...
export function initUi(): void {
	...
	$("#stage").addEventListener(
		...
		(e) => {
			...
			setZoom(app.zoom * 1.0025 ** -e.deltaY, {
				sx: e.clientX - r.left,
				sy: e.clientY - r.top,
			});
		},
