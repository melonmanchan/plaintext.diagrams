...
function translate(s: Shape, dx: number, dy: number): void {
  if (s.type === 'arrow') {
    ...
    if (s.box1 == null) { s.x1 += dx; s.y1 += dy; }
    if (s.box2 == null) { s.x2 += dx; s.y2 += dy; }
  } else {
    s.x += dx;
    s.y += dy;
  }
...
export function autoLayout(shapes: Shape[], only?: ReadonlySet<number>): void {
  ...
  const nodes: (BoxShape | GroupShape)[] = groups.filter(
    (g) => pick(g.id) && !groups.some((o) => o !== g && pick(o.id) && insideGroup(g, o)),
  );
  ...
  for (const n of nodes) {
    ...
    carried.set(n.id, shapes.filter((s) => s.id !== n.id && insideGroup(s, n)));
  }
