...
function boot(): void {
	...
	try {
		idx = JSON.parse(localStorage.getItem(STORE_INDEX) ?? "null");
	} catch {
		/* corrupt index */
	}
...
async function loadAppFonts(): Promise<void> {
	try {
		await document.fonts.load(
			FONT,
			"plaintext.diagrams Browser Web Server Database",
		);
		await document.fonts.ready;
	} catch {
		/* render with fallback fonts if the Font Loading API is unavailable */
	}
}
...
const appHook = {
	...
	shareLink: async () => {
		...
		return (
			...
			location.pathname +
			"#s=" +
			(await encodeShareLink(name, app.doc.shapes))
		);
	},
};
