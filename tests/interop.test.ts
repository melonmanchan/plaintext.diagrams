...
describe('shapes JSON interop', () => {
  ...
  it('rejects duplicate explicit ids', () => {
    const r = parseShapesJson('[{"type":"box","id":1,"x":0,"y":0,"w":8,"h":3},{"type":"box","id":1,"x":20,"y":0,"w":8,"h":3}]');
  ...
  it('rejects non-numeric coordinates and dimensions', () => {
    expect(parseShapesJson('[{"type":"box","x":"abc","y":0,"w":8,"h":3,"text":"A"}]')!.errors[0])
      .toContain('"x" and "y" must be numbers');
    expect(parseShapesJson('[{"type":"box","x":0,"y":0,"w":"8","h":3,"text":"A"}]')!.errors[0])
      .toContain('"w" and "h" must be numbers');
    expect(parseShapesJson('[{"type":"arrow","x1":"a","y1":0,"x2":5,"y2":5}]')!.errors[0])
      .toContain('x1,y1,x2,y2 must be numbers');
  });

  it('rejects lanes that are not an array of strings', () => {
    const r = parseShapesJson('[{"type":"group","x":0,"y":0,"w":30,"h":10,"lanes":[1,2]}]');
  ...
  it('clamps oversized geometry to the world instead of freezing render', () => {
    const r = parseShapesJson('[{"type":"box","id":1,"x":0,"y":0,"w":100000,"h":100000,"text":"A"},' +
